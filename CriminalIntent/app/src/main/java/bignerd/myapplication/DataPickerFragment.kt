...
class DataPickerFragment: DialogFragment() {
    ...
    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        val dateListener = DatePickerDialog.OnDateSetListener { _:DatePicker,year:Int,month:Int,day:Int ->
            ...
            val result = Bundle().apply { putSerializable(RESULT_DATE_KEY,resultDate) }
    ...
    companion object{
        fun newInstance(date: Date,requestCode: String):DataPickerFragment{
            val args = Bundle().apply { putSerializable(ARG_DATE,date)
                putString(ARG_REQUEST_CODE, requestCode)}

            return DataPickerFragment().apply { arguments = args }
        }
